...
const initScene = () => {
  ...
  scene.background = new THREE.Color('#091225');
  scene.fog = new THREE.Fog('#091225', 18, 42);

  camera = new THREE.PerspectiveCamera(38, 1, 0.1, 120);
  camera.position.set(2.6, 10.5, 3.35);
...
const createRoomShell = () => {
  ...
  addBox('Wall_Top_Structure_Beam', [ROOM.width + 0.3, 0.18, 0.18], [0, h + 0.16, -halfD], trimMaterial);
  addBox('Wall_Left_Structure_Beam', [0.18, 0.18, ROOM.depth], [-halfW, h + 0.16, 0], trimMaterial);
  addBox('Wall_Right_Structure_Beam', [0.18, 0.18, ROOM.depth], [halfW, h + 0.16, 0], trimMaterial);
  addBox('Wall_Bottom_Structure_Beam', [ROOM.width + 0.3, 0.18, 0.18], [0, h + 0.16, halfD], trimMaterial);

  addLightEdge('Wall_Light_Edge_Left_Corner', [0.05, h + 0.3, 0.05], [-halfW + 0.08, 1.48, -halfD + 0.08], blueLineMaterial);
  addLightEdge('Wall_Light_Edge_Right_Corner', [0.05, h + 0.3, 0.05], [halfW - 0.08, 1.48, -halfD + 0.08], blueLineMaterial);
  addLightEdge('Wall_Light_Edge_Bottom_Left', [0.05, h * 0.55, 0.05], [-halfW + 0.08, 0.86, halfD - 0.08], blueLineMaterial);
  addLightEdge('Wall_Light_Edge_Bottom_Right', [0.05, h * 0.55, 0.05], [halfW - 0.08, 0.86, halfD - 0.08], blueLineMaterial);
...
.lab-model-viewer {
  ...
  background:
    ...
    linear-gradient(180deg, #0b1730 0%, #050b18 100%);
}
