<template>
  ...
  >
    <button class="mascot-trigger" type="button" aria-label="打开智小喵 AI 问答" @click="openDialog">
      ...
      <img src="/assets/mascot-cutout.png" alt="智小喵 AI 助手" draggable="false" />
  ...
  <Teleport to="body">
      <section
        v-if="dialogOpen"
        class="ai-dialog glass-panel"
        :style="dialogStyle"
        role="dialog"
        aria-modal="false"
        aria-label="智小喵 AI 问答对话框"
        @pointerdown.stop
      >
        ...
        <div class="dialog-body">
          <aside class="dialog-sidebar">
            <img src="/assets/mascot-cutout.png" alt="智小喵" class="dialog-mascot" draggable="false" />
...
const dialogStyle = computed(() => {
  ...
  const left = Math.min(Math.max(12, position.x + 148), window.innerWidth - width - 12);
  const top = Math.min(Math.max(82, position.y - 330), window.innerHeight - height - 12);
...
const buildReply = (question) => {
  ...
  if (normalized.includes('成员') || normalized.includes('老师') || normalized.includes('学生')) {
    return {
      text: '实验室目前共有 34 名成员，包括 2 名导师、8 名研究生和 24 名本科生。你也可以继续问某个方向或成员画像。',
      cards: [
        { icon: '👨‍🏫', title: '导师团队', meta: '2人' },
        { icon: '🎓', title: '研究生', meta: '8人' },
        { icon: '👥', title: '本科生', meta: '24人' }
      ]
    };
  }
  if (normalized.includes('活动') || normalized.includes('安排') || normalized.includes('会议')) {
    return {
      text: '近期重点活动包含项目评审会、算法团队例会和成果汇报会。需要查看日程时，可以切到活动风采或空间设备页。',
      cards: [
        { icon: '📅', title: '团队例会', meta: '10:00-11:30' },
        { icon: '📌', title: '项目评审会', meta: '14:00-15:30' },
        { icon: '🏆', title: '成果汇报会', meta: '16:00-17:30' }
      ]
    };
  }
  return {
...
.mascot-trigger {
  ...
  filter: drop-shadow(0 10px 18px rgba(0, 72, 160, 0.32)) drop-shadow(0 0 14px rgba(0, 168, 255, 0.42));
}
...
.ai-dialog {
  ...
  flex-direction: column;
  overflow: hidden;
  box-shadow: 0 18px 52px rgba(0, 8, 24, 0.58), 0 0 26px rgba(56, 189, 248, 0.24);
}
